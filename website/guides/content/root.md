...
顶部状态栏显示“有效温度”和“目标转速”。风扇卡片上的百分比在自动、曲线模式下表示目标调速值，在手动模式下表示当前设置值。RPM 表示测得的每分钟转数，只有取得有效读数时才显示。百分比与 RPM 的含义不同。自动模式还可能显示“按曲线运行”“自适应调速”“保护介入”或“温度失效”，以及安全参考温度、45 秒预测温度和升温速度。预测温度用于提前调速。出现“温度失效”时，先检查温度来源是否持续更新；设备会采用保护转速。点击卡片上的信息按钮，可以查看自动模式说明。
...
- “色彩校正”：调整矩阵显示效果，并根据页面按钮重置、导入或导出校正设置。LED 矩阵的尺寸、动画和滤镜由当前设备决定。文档中的操作以页面实际显示内容为准。

## 6. 网络管理

点击顶部导航中的“网络”进入“网络设置”。修改网络模式、热点或 NAT 设置可能中断当前WebUI、终端和自动化连接。保存设置前，应确认可以通过新的网络地址重新连接设备。
...
名称用于帮助你辨认指令，ID 用于自动化引用。编辑现有指令时，ID 为只读，不能直接修改。需要使用新 ID 时，请新建指令，并检查相关动作模板和数据源的引用。
...
- “超时”：超过等待时间仍未得到匹配结果时停止等待。该设置仅在配置成功或失败匹配模式，或启用“匹配后自动停止”时生效。
...
nohup 表示命令在远程主机后台继续运行，SSH 断开后仍可继续。后台任务不会因为关闭 WebUI自动停止。启用“后台执行（nohup）”后，可以使用：
...
“服务模式（监测就绪状态）”用于持续观察后台任务是否进入可用状态。启用后，必须填写“就绪匹配模式”和“存储变量名”；失败匹配和等待设置可按需要调整：
...
| “暂停” | 暂停后续自动判断，保留配置；已开始的动作可能继续执行。恢复时先点“停止”，再点“启动”。 |
| “停止” | 停止规则处理和数据源读取。已有配置不会因此删除。 |
| “重新加载” | 重新读取已保存的引擎配置；正在运行的引擎会在加载后恢复运行。请先保存编辑内容。 |

操作前，确认散热、告警或其他持续任务是否依赖自动化。暂停和停止都不能保证终止已提交的异步动作或远程后台进程；请到对应任务页面检查。操作后刷新状态，确认结果。
...
3. 对 REST API、WebSocket 或 Socket.IO，从测试结果中选择需要保存的数据字段。Socket.IO的事件名称留空时，测试功能可以尝试发现服务推送的事件。
...
页面提供的比较方式包括：等于、不等于、大于、大于等于、小于、小于等于、值变化和包含。比较值应与变量的数据类型相符。
...
- “单次”：规则每次触发时执行一次。
- “条件持续时重复”：按单个动作的执行条件判断，每次间隔后继续；每轮最多执行 100 次。
- “指定次数”：按设置次数和间隔重复。

单个动作的执行条件与整条规则的触发条件分别设置。使用“条件持续时重复”前，请为该动作设置执行条件；留空会持续重复到每轮次数上限。达到上限后，这条规则仍可能再次触发。

停用规则只阻止后续自动触发，已经开始的一轮重复动作可能继续。需要停止时，先停用规则，检查正在执行的动作；远程后台任务请到“指令”页面停止对应进程，并确认进程已退出。
...
2. 勾选“仅手动触发”。
3. 添加需要执行的动作模板。
4. 保存并启用规则，卡片才会出现在系统页。
5. 返回“系统”，在“快捷操作”中检查卡片并执行一次，并检查结果。
...
- 手动触发会执行规则动作，且不会受规则启用状态或冷却时间限制。即使规则已停用，也不要将手动触发当作只读检查。
...
The AGX and LPMU buttons show the current state. Green indicates a running device, red indicates a powered-off device, and a waiting state appears during detection. Forced power-off can cause loss of unsaved data. Save work on the module and complete its normal shutdown process before removing power.
...
5. Review the advanced options, then select “Save.”The name helps you recognize a command; Automation uses its ID to refer to it. The ID is read-only when you edit an existing command. To use a different ID, create a new command and update any action templates or data sources that should use it.
...
- “Timeout”: Stops waiting when no match arrives within the configured time. It applies only when a success or failure pattern is set, or “Stop on Match” is enabled.
...
- “Stop Process”: End the corresponding background task. “Service Mode (monitor ready state)” watches a background task until it becomes available. You must provide a Ready Pattern and Variable Name. Adjust the optional failure pattern and timing settings as needed:
...
```text
Automatic: Data source → Variable → Rule → Action
Manual: System Quick Action → Action
```
...
| “Pause” | Pauses further automatic evaluation. Actions already underway may continue. To resume, select “Stop,” then “Start.” |
| “Stop” | Stops rule processing and data-source updates. Saved configuration is retained. |
| “Reload” | Reloads the saved engine configuration. If the engine was running, it resumes afterward. Save your edits first. |

Before changing the engine state, check whether cooling, alerts, or other ongoing tasks depend on Automation. Pause and Stop do not guarantee that queued asynchronous actions or remote background processes will end. Check those tasks separately, then refresh the engine status.
...
4. Create a rule with “Enable immediately” cleared.
...
“Test” executes the action immediately. Confirm the effect before testing power controls, reboots, remote commands, LED output, or external requests. Add an action to a rule only after the test result is understood. Before deleting an action template, confirm that no rule references it. Import can overwrite an action with the same ID. Follow the Security Guide for trust requirements when importing or exporting configuration packages.
...
3. Choose “Logic”:
- Choose AND to trigger when all conditions match.
- Choose OR to trigger when any condition matches.
...
- “Repeat while true”: Repeats at the configured interval while that action’s own condition is true, up to 100 executions per run.
- “Fixed count”: Repeats for the configured count and interval.

An action’s execution condition is separate from the rule’s trigger conditions. Set an action condition before using “Repeat while true.” Without one, the action repeats until it reaches the per-run limit. The rule may trigger another run afterward.

Disabling a rule prevents future automatic triggers; a repeat sequence already in progress may continue. Disable the rule first, then check the active actions. For a remote background task, stop its process on the Commands page and confirm that it has exited.
...
4. Save and enable the rule so its card appears on the System page.
5. Return to “System,” find the card under “Quick Actions,” and run it once and check the result.
...
- Manual Trigger runs the actions without checking whether the rule is enabled or waiting for its cooldown. It is not a preview, even when the rule is disabled.
